...
(function (root, factory) {
   ...
   */
  var TextIconOverlay = function (position, text, options, cluster) {
    try {
      BMap;
    } catch (e) {
    ...
    if (options && options['styles'] && options['styles'][0] && options['styles'][0].class) {
      this._cssClass = options['styles'][0].class
    } else {
      this._cssClass = 'cluster_class_default'
    }
  };
   ...
   *@return 无返回值。
   */
  TextIconOverlay.prototype._updateCssClass = function (newClass) {
    if (this._domElement) {
      this._domElement.className = newClass ? newClass :this._cssClass;
    }
  };
  ...
  TextIconOverlay.prototype._updatePosition = function () {
    if (this._domElement && this._position) {
      ...
      pixelPosition.x -= Math.ceil(parseInt(style.width ? style.width : 0) / 2);
      pixelPosition.y -= Math.ceil(parseInt(style.height ? style.height : 0) / 2);
  ...
  TextIconOverlay.prototype._buildCssText = function (style) {
    //根据style来确定一些默认值
    var isClassUse = style['isClassUse'];
    var url = style['url'];
    var size = style['size'];
    ...
    if (!isClassUse) {
      ...
      csstext.push('cursor:pointer; position:absolute;')
    }
  ...
  TextIconOverlay.prototype._bind = function () {
    ...
    T.event.on(this._domElement, 'mouseover', function (e) {
      me._vmthis && me._vmthis.$emit('mouseover', {
				point: me._position,
        events: e,
        cluster: me._cluster
			})
    ...
    T.event.on(this._domElement, 'mouseout', function (e) {
      me._vmthis && me._vmthis.$emit('mouseout',e)
    });
    T.event.on(this._domElement, 'mouseleave', function (e) {
      me._vmthis && me._vmthis.$emit('mouseleave',e)
    });
    T.event.on(this._domElement, 'contextmenu', function (e) {
      me._vmthis && me._vmthis.$emit('contextmenu',e)
    });
    T.event.on(this._domElement, 'click', function (e) {
			me._vmthis && me._vmthis.$emit('click',e)
      me.dispatchEvent(eventExtend(e, new BaseEvent('onclick')),e);
    });
  };
